...
$stats = [
  ...
  'food_pending'    => (int)$conn->query("SELECT COUNT(*) c FROM food_donations WHERE status='pending'")->fetch_assoc()['c'],
  'food_delivered'  => (int)$conn->query("SELECT COUNT(*) c FROM food_donations WHERE status='delivered'")->fetch_assoc()['c'],
  'cloth_total'     => (int)$conn->query("SELECT COUNT(*) c FROM cloth_donations")->fetch_assoc()['c'],
  'cloth_pending'   => (int)$conn->query("SELECT COUNT(*) c FROM cloth_donations WHERE status='pending'")->fetch_assoc()['c'],
  'cloth_delivered' => (int)$conn->query("SELECT COUNT(*) c FROM cloth_donations WHERE status='delivered'")->fetch_assoc()['c'],
  ...
  'contact_msgs'    => (int)$conn->query("SELECT COUNT(*) c FROM contact_messages")->fetch_assoc()['c'],
];
...
$pending_food  = $conn->query("SELECT id,donor_email,quantity,pickup_address,priority,created_at FROM food_donations WHERE status='accepted' ORDER BY priority DESC,created_at ASC LIMIT 20")->fetch_all(MYSQLI_ASSOC);
$pending_cloth = $conn->query("SELECT id,donor_email,quantity,pickup_address,created_at FROM cloth_donations WHERE status='accepted' ORDER BY created_at ASC LIMIT 20")->fetch_all(MYSQLI_ASSOC);

// Settlements (safe if table doesn't exist yet)
$settlements_exist = $conn->query("SHOW TABLES LIKE 'settlements'")->num_rows > 0;
...
$food        = $conn->query("SELECT * FROM food_donations ORDER BY created_at DESC LIMIT $per OFFSET ".(($fp-1)*$per));
$cloth       = $conn->query("SELECT * FROM cloth_donations ORDER BY created_at DESC LIMIT $per OFFSET ".(($cp-1)*$per));
...
$returns  = $conn->query("SELECT rr.*,p.name AS product_name FROM return_requests rr JOIN products p ON p.id=rr.product_id ORDER BY rr.created_at DESC")->fetch_all(MYSQLI_ASSOC);
$contacts = $conn->query("SELECT * FROM contact_messages ORDER BY created_at DESC LIMIT 30")->fetch_all(MYSQLI_ASSOC);

// ── Events & News ─────────────────────────────────────────────────────────
$events_news = $conn->query("SELECT * FROM events_news ORDER BY created_at DESC LIMIT 50")->fetch_all(MYSQLI_ASSOC);
$events_count = count($events_news);
$stats['events_news'] = $events_count;

// ── Activity feed ─────────────────────────────────────────────────────────
$activity = $conn->query("
  SELECT 'food' AS atype, donor_email AS actor, status, created_at, id FROM food_donations
  UNION ALL SELECT 'cloth', donor_email, status, created_at, id FROM cloth_donations
  UNION ALL SELECT 'order', buyer_email, order_status, created_at, id FROM orders
  ORDER BY created_at DESC LIMIT 8
")->fetch_all(MYSQLI_ASSOC);

// ── Donation status distribution ─────────────────────────────────────────
$don_statuses = $conn->query("SELECT status,COUNT(*) c FROM (SELECT status FROM food_donations UNION ALL SELECT status FROM cloth_donations) x GROUP BY status ORDER BY c DESC")->fetch_all(MYSQLI_ASSOC);
